...
export const changelog: ChangeEntry[] = [
  {
    version: '1.6.0',
    date: '2026-09-16',
    title: 'Profile-Aware AI Chat — Multi-Provider with Failover',
    summary:
      'Floating chat widget grounded to Ganesh\'s profile and Learning Hub. Visitors can ask about experience, skills, projects, and Java/Kafka concepts. Multi-provider architecture with automatic failover on rate limits — Groq (primary) falls back to Hugging Face Router seamlessly.',
    type: 'feature',
    changes: [
      'Floating "Ask AI" pill button (bottom-right) — matches LearningHub entry style exactly.',
      'Multi-provider architecture: Groq (primary, qwen/qwen3.8-27b) → HF Router (fallback, Qwen/Qwen3.8-27B) with automatic failover on 429/503.',
      'NVIDIA NIM (nemotron-3.5-lightning) loaded for future server-side use — browser CORS blocks it, but a Vercel Edge Function can proxy it.',
      ...
      'System prompt with guardrails: model refuses off-topic questions, never invents information, stays grounded to profile data.',
      'Real SSE streaming — tokens appear live as the model generates them (OpenAI-compatible format, shared parser across providers).',
      '5-second client-side cooldown prevents hitting Groq\'s 30 req/min free-tier limit.',
      ...
      'Accessibility: ARIA labels, keyboard nav (Enter to send, Shift+Enter for newline), focus management.',
      'Graceful degradation: widget hidden entirely when no provider token is set.',
      'Configurable via VITE_GROQ_TOKEN, VITE_HF_TOKEN, VITE_NVIDIA_TOKEN. See .env.example.',
    ],
  },
  {
    version: '1.5.0',
